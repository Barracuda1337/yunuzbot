...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "9B0Ixer8sEM1xpLF_lPK3ivAaUPaKRVB",
    callbackURL: `https://yunuzbot.glitch.me/callback`,
    domain: `https://yunuzbot.glitch.me/`
  };
...
//Tovbe tovbe kufurlere bak 
 ...
 const renderTemplate = (res, req, template, data = {}) => {
  ...
  app.get("/callback", passport.authenticate("discord", { failureRedirect: "/autherror" }), async (req, res) => {
    ...
    client.channels.get("697934515720421507").send(`**${client.users.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
...
res.redirect("/panel/"+req.params.guildID+"/genel");
  ...
  app.get("/ekle", (req, res) => {
   ...
   app.get("/seviye", (req, res) => {
    res.redirect(`https://yunuzbot.glitch.me/kullanici/${req.params.kullaniciID}/yonet`);
  });
